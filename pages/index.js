...
export default function Home(props) {
  ...
  */
	...
	async function buyMarketNfts(nft) {
		...
		const transaction = await contract.MarketSale(nftcreator, nft.tokenId, {
			value: price,
		});
...
export async function getServerSideProps() {
	const res = await fetch(process.env.NEXT_PUBLIC_API_SERVER).then((response) =>
		response.json()
	);
